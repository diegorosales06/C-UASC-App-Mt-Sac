 ...
 * Find WAYPOINT_DWELL_MS at the top of this file. Change the value in milliseconds.
 * 10_000L = 10 seconds. 5_000L = 5 seconds. etc.
 *
 * ── Offline debug mode ────────────────────────────────────────────────────────
 * When OfflineDebugConfig.OFFLINE_DEBUG_MODE = true:
 *   - No DJI SDK calls are made.
 *   - The simulated drone position is updated mathematically each control tick.
 ...
 *   horizontalSpeed = Kp * distance − Kd * distanceRate   (clamped to [MIN, MAX])
 *   verticalSpeed   = Kp_VERTICAL * altError               (clamped to ±MAX_VERTICAL)
...
public class WaypointMissionController {
    ...
    // ── Dwell duration ────────────────────────────────────────────────────────
    // Time the drone hovers at each waypoint before proceeding to the next one.
    // Change this value to adjust the stop duration. Units: milliseconds.
    private static final long WAYPOINT_DWELL_MS = 10_000L;

    // ── Control loop timing ───────────────────────────────────────────────────
    private static final long  CONTROL_LOOP_INTERVAL_MS = 50;
    private static final float DT = CONTROL_LOOP_INTERVAL_MS / 1000.0f; // seconds per tick
    ...
    // ── GPS state — updated by flight controller state callback ───────────────
    ...
    // ── Main-thread handler for dwell countdown and UI posts ──────────────────
    private final Handler mainHandler = new Handler(Looper.getMainLooper());

    // ── FlightLogger ──────────────────────────────────────────────────────────
    private FlightLogger flightLogger;

    // ── Countdown runnable — re-posts itself every second during dwell ────────
    ...
    // =========================================================================
    // Public API — called by VirtualStickWaypointView button handlers
    // =========================================================================

    /**
     * Initialises the DJI FlightController and attaches the GPS state callback.
     * Safe to call multiple times — re-attaches if the controller reference was lost.
     * In offline debug mode this sets up a simulated start position instead.
     */
    public void initFlightController() {
        if (OfflineDebugConfig.OFFLINE_DEBUG_MODE) {
            ...
            callback.onLogMessage("Offline debug mode enabled. Drone/controller connection is skipped.");
            return;
        }
     ...
     * Validates preconditions, enables Virtual Stick mode, resets state, starts control loop.
     */
     ...
     * Cancels any active dwell, sends zero-velocity hover, disables Virtual Stick.
     */
     ...
     *
     * If isDwelling, only hover commands are sent — all navigation math is skipped.
     * Otherwise: compute distance + bearing, run PD controller, send velocity command.
     */
    private void runControlStep() {
        if (!missionRunning || !hasValidGPS) return;
        if (currentWaypointIndex >= store.size()) return;

        // ── Dwell phase: just hover until the timer fires ─────────────────────
        if (isDwelling) {
            sendVelocityCommand(0, 0, 0);
            if (OfflineDebugConfig.OFFLINE_DEBUG_MODE) {
        ...
        // ── Waypoint acceptance ───────────────────────────────────────────────
        boolean horizontalReached = distanceM <= WaypointNavigationMath.ACCEPTANCE_RADIUS_M;
        boolean verticalReached   = Math.abs(altError) <= WaypointNavigationMath.ALTITUDE_ACCEPTANCE_M;
        ...
        // ── PD + feedforward horizontal speed controller ──────────────────────
        ...
        if (distanceM > brakingDist) {
            // PHASE 1 — CRUISE: full speed, no P/D computation needed
     ...
     * Stops the drone, marks dwell active, starts the countdown.
     */
    private void enterDwell() {
        ...
        // Start the per-second countdown on the main thread
        startCountdown(wpNumber);

        // Schedule the end of the dwell
        mainHandler.postDelayed(() -> exitDwell(), WAYPOINT_DWELL_MS);
    }

    /**
     * Fires once per second during the dwell to update the status countdown label.
     */
    private void startCountdown(int wpNumber) {
        ...
        countdownRunnable = new Runnable() {
            ...
            public void run() {
                ...
                long remainingSec = Math.max(0, (remainingMs + 999) / 1000); // ceiling
                callback.onStatusChanged(String.format(
                        "Mission: DWELLING - WP%d (%ds remaining)", wpNumber, remainingSec));
                if (remainingSec > 0) {
    ...
    /** Cancels any pending countdown Runnable. */
    ...
    private void exitDwell() {
        if (!missionRunning) return; // mission was stopped during dwell
        isDwelling = false;
        cancelCountdown();
     ...
     *
     * @param pitch    East velocity  (m/s) — positive = fly East   (DJI GROUND mode)
     * @param roll     North velocity (m/s) — positive = fly North  (DJI GROUND mode)
     ...
     * (or a hardcoded CSULA position if the waypoint list is empty).
     */
    ...
    private void startOfflineDebugMission() {
        ...
        callback.onLogMessage("Offline debug mission started. Simulating waypoint travel.");
        startControlLoop();
    }
    ...
    /**
     * Moves the simulated drone position by integrating the velocity commands over DT.
    ...
    private void applyOfflineDebugVelocity(float pitchVelocity,
        ...
        // In DJI GROUND mode: pitch = east, roll = north
    ...
    /** Builds the "Target WP N: (lat, lng) @ alt m" label string. */
